...
const { FiEdit3, FiHome, FiTrendingUp, FiHeart, FiMail } = FiIcons;
...
const Header = () => {
  ...
  const navItems = [
    ...
    { path: '/emails', icon: FiMail, label: 'My Emails' },
          ...
          <nav className="flex items-center space-x-1">
            {navItems.map((item) => {
              const isActive = location.pathname === item.path || 
                               (item.path === '/emails' && location.pathname.startsWith('/write/edit'));
              return (
                <Link
                  key={item.path}
                  to={item.path}
