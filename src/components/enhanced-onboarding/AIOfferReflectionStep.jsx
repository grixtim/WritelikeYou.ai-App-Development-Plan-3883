...
const { FiMessageCircle, FiArrowRight, FiArrowLeft, FiEdit, FiLoader } = FiIcons;

const AIOfferReflectionStep = ({ data, onUpdate }) => {
  ...
  const generateReflection = async () => {
    setIsLoading(true);
    try {
      const response = await willyAPI.generateSetupReflection(data);
      setReflection(response.reflection);
    } catch (error) {
      console.error('Error generating reflection:', error);
      // Fallback to a generic reflection
      setReflection(generateFallbackReflection());
    } finally {
      setIsLoading(false);
    }
  };

  const generateFallbackReflection = () => {
    return `So your offer, ${data.offerName}, is a ${data.offerDescription} designed for people who currently ${data.clientCurrentIdentity.split('.')[0].toLowerCase()}. Through your work together, they'll transform into someone who ${data.clientDesiredIdentity.split('.')[0].toLowerCase()}, overcoming obstacles like ${data.commonObstacles.slice(0, 2).join(', ')}. Most importantly, they'll achieve what they truly want: ${data.mainResults.slice(0, 2).join(', ')}.`;
  };
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 md:p-12">
        ...
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-4">
            Willy's reflecting on your offer...
          </h1>
          <p className="text-lg text-gray-600 leading-relaxed">
            Give me just a moment to understand what you've shared and put it into words
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex space-x-4 pt-6">
            ...
            <button
              onClick={() => {
                onUpdate(editedData);
                setIsEditing(false);
                generateReflection(); // Regenerate reflection with new data
              }}
              className="flex-1 flex items-center justify-center space-x-2 py-4 rounded-xl font-semibold text-lg transition-all bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-lg hover:shadow-xl"
            >
...
export default AIOfferReflectionStep;